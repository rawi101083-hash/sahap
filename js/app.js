...
window.appLogic = {
    ...
    generateThermalHTML(data, qrContainerId) {
        ...
        (data.items || []).forEach(item => {
            ...
            itemsHtml += `
            <tr>
                <td style="padding:4px 0; border-bottom:1px dashed #bbb; text-align:right; width:50%; word-wrap:break-word; white-space:normal; font-size:11px;">${item.name} <div style="font-size:10px; color:#555;">- ${tLbl} - ${sLbl}</div></td>
                <td style="padding:4px 0; border-bottom:1px dashed #bbb; text-align:center; width:20%; font-size:11px;">${item.qty}</td>
                <td style="padding:4px 0; border-bottom:1px dashed #bbb; text-align:left; direction:ltr; width:30%; font-size:11px;">${(item.unitPrice * item.qty).toLocaleString(displayLocale, displayOpts)}</td>
            </tr>`;
        });
        if (deliveryFee > 0) {
            itemsHtml += `
            <tr>
                <td style="padding:4px 0; border-bottom:1px dashed #bbb; text-align:right; width:50%; word-wrap:break-word; white-space:normal; font-size:11px;">رسوم التوصيل</td>
                <td style="padding:4px 0; border-bottom:1px dashed #bbb; text-align:center; width:20%; font-size:11px;">1</td>
                <td style="padding:4px 0; border-bottom:1px dashed #bbb; text-align:left; direction:ltr; width:30%; font-size:11px;">${deliveryFee.toLocaleString(displayLocale, displayOpts)}</td>
            </tr>`;
        }
        ...
        return `
        <style>
            .receipt-header { text-align:center; border-bottom:1px solid #000; padding-bottom:8px; margin-bottom:10px; width: 100%; box-sizing: border-box; }
            ...
            .print-table { width: 100% !important; table-layout: fixed; border-collapse: collapse; font-size: 11px; }
            .print-table td, .print-table th { word-wrap: break-word; white-space: normal; }
        </style>
        <div style="width: 100% !important; max-width: 70mm !important; margin:0 auto; padding:2mm; box-sizing:border-box; overflow:hidden; font-family:'Tajawal',Arial,sans-serif; direction:rtl; background:#fff; color:#000;">
            ...
            <div class="receipt-header">
                ...
                <div style="font-size:16px; font-weight:900; margin-bottom:4px; word-wrap:break-word;">${(window.tenantSettings || {}).name || ''}</div>
                <div style="font-size:11px; font-weight:bold; margin-bottom:4px;">${_hasValTax ? 'فاتورة ضريبية مبسطة' : 'فاتورة مبيعات'}</div>
                ${storeAddress}
                ${storeTax}
                ${storePhone}
            </div>

            <!-- Meta info — Flex container for safety against overflow -->
            <div style="display:flex; flex-wrap:wrap; width:100%; box-sizing:border-box; font-size:11px; margin-bottom:8px; direction:rtl;">
                <div style="width:100%; display:flex; justify-content:space-between; border-bottom:1px solid #eee; padding:3px 0;">
                    <span style="color:#555;">رقم الفاتورة:</span>
                    <span style="font-weight:bold; direction:ltr; word-wrap:break-word; max-width:60%; text-align:left;">${_invId}</span>
                </div>
                <div style="width:100%; display:flex; justify-content:space-between; border-bottom:1px solid #eee; padding:3px 0;">
                    <span style="color:#555;">التاريخ:</span>
                    <span style="direction:ltr; word-wrap:break-word;">${dStr}</span>
                </div>
                ${_custName ? `<div style="width:100%; display:flex; justify-content:space-between; border-bottom:1px solid #eee; padding:3px 0;"><span style="color:#555;">العميل:</span><span style="font-weight:bold; word-wrap:break-word; max-width:70%; text-align:left;">${_custName}</span></div>` : ''}
                ${data.customer.phone && data.customer.phone !== '0000000000' ? `<div style="width:100%; display:flex; justify-content:space-between; border-bottom:1px solid #eee; padding:3px 0;"><span style="color:#555;">رقم الجوال:</span><span style="font-weight:bold; direction:ltr; word-wrap:break-word;">${data.customer.phone}</span></div>` : ''}
                <div style="width:100%; display:flex; justify-content:space-between; padding:3px 0;">
                    <span style="color:#555;">وقت الطباعة:</span>
                    <span style="direction:ltr; color:#333;">${new Date().toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true })}</span>
                </div>
            </div>
            ...
            <table class="print-table">
                <thead>
                    <tr style="border-top:1px solid #000; border-bottom:1px solid #000;">
                        <th style="padding:3px 0; text-align:right; width:50%;">الصنف</th>
                        <th style="padding:3px 0; text-align:center; width:20%;">م</th>
                        <th style="padding:3px 0; text-align:left; width:30%;">إجمالي</th>
                    </tr>
                </thead>
            ...
            <table class="print-table" style="margin-top:8px;">
                ${_hasValTax ? `
                <tr>
                    <td style="padding:3px 0; color:#555; width:60%;">المجموع بدون ضريبة</td>
                    <td style="text-align:left; font-weight:bold; direction:ltr; width:40%;">${combinedSum.toLocaleString(displayLocale, displayOpts)} ر.س</td>
                </tr>
                <tr>
                    <td style="padding:3px 0; color:#555; width:60%;">ضريبة القيمة المضافة 15٪</td>
                    <td style="text-align:left; font-weight:bold; direction:ltr; width:40%;">${vatAmount.toLocaleString(displayLocale, displayOpts)} ر.س</td>
                </tr>` : ''}
                <tr>
                    <td style="padding:3px 0; color:#555; width:60%;">طريقة الدفع</td>
                    <td style="text-align:left; font-weight:bold; width:40%;">${{ 'cash': 'نقدي', 'mada': 'مدى', 'visa': 'فيزا', 'mastercard': 'ماستركارد', 'network': 'شبكة' }[data.paymentMethod] || data.paymentMethod}</td>
                </tr>
                <tr style="border-top:1px solid #000;">
                    <td style="padding:5px 0; font-size:12px; font-weight:900; width:50%;">${_hasValTax ? 'الإجمالي الشامل' : 'الإجمالي'}</td>
                    <td style="text-align:left; font-size:12px; font-weight:900; direction:ltr; width:50%;">${grandTotal.toLocaleString(displayLocale, displayOpts)} ر.س</td>
                </tr>
            </table>
            ...
            <div style="text-align:center; margin-top:12px; border-top:1px solid #ccc; padding-top:10px;">
                ...
                <div style="font-size:10px; color:#555;">Thank you for choosing Sahab POS</div>
            </div>
